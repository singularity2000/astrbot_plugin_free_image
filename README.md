...
| 模态筛选 | 每个 API 节点可勾选文生图、图生图、文生视频、图生视频能力；请求只会调用支持当前模态的节点。模板和自拍按图生图处理。 |
...
| 配置项 | 说明 |
| :--- | :--- |
| `api_pipeline` | 图片生成 Provider 管线，决定模型、API 地址、Key、代理、超时、重试和模型能力。 |
...
| `general.generation_start_message` | 自定义开始文案，留空用默认；支持 `{type}`、`{prompt}`，也可填写纯文字。 |
| `general.ignore_bot_at_avatar` | 普通图生图/模板是否忽略机器人自身的 @头像，默认关闭；自拍始终忽略。 |
...
| `#画图缓存 状态/开启/关闭/清理` | 查看、开关或清理生成图片缓存。清理只删除未收藏的缓存图片，保留收藏图片及生图历史记录。 | `#画图缓存 状态` |
...
在 AstrBot 原生配置的“通用”或 Pages“设置 → 通用”中编辑“生图开始提示文案”，两处使用同一配置。

- 留空或仅空白：普通生图显示 `🎨 收到文生图/图生图请求，正在生成 [描述]…`，自拍命令显示 `📸 正在生成自拍 [人设名称]…`。
- `{type}`：文生图、图生图、自拍、文生视频、图生视频；后两种为预留文案，视频生成能力敬请期待。
- `{prompt}`：显示用描述摘要（最多 20 个字符，超出加 `…`）；普通生图取提示词/模板显示名称，自拍取动作描述，未填写动作时取人设名称。
- 示例：`✨ {type}开始了：{prompt}`；也可以直接填 `好的，我马上处理。`。没有占位符时按原文发送，未知占位符原样保留，不执行表达式。
- 仅替换原本会发送的开始文字，不改变权限、配额、引用回复和简洁模式；`send_selfie` 始终不发送开始文字。
...
- 统计记录支持提示词关键词搜索，可组合日期、触发人、模式、模型和“仅看收藏”筛选。
- 记录左侧可勾选，表头全选仅针对当前页；翻页或重新查询会清空勾选。删除所选记录需要确认，并同时删除关联本地生成图片（包括收藏）；不会删除自拍参考图。
- 缩略图及生成图片全屏预览左上角的星形按钮可收藏单张图片。收藏图片不计入数量、大小、时间限制，也不会被 Pages / 命令一键清理；显式删除图片或所属记录仍会删除收藏。取消收藏后重新受缓存限制约束，后续清理可能删除它。收藏仍占用磁盘空间，界面分别展示普通缓存及收藏的数量与占用。
